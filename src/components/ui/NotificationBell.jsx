...
function NotificationBell() {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {/* Dropdown Card */}
            {isOpen && (
                <div className="fixed sm:absolute top-12 left-2 right-2 sm:left-auto sm:-right-12 md:right-0 w-auto sm:w-80 md:w-96 max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden transform opacity-100 scale-100 origin-top-right transition-all">

                    {/* Header */}
                    <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                    ...
                    <div className="max-h-[60vh] sm:max-h-[70vh] overflow-y-auto custom-scrollbar">
                        {loading && notifications.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-slate-400 px-4">
                                ...
                                <p className="text-xs sm:text-sm">Loading alerts...</p>
                            </div>
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                {notifications.map((notif) => (
                                    <div
                                        key={notif.id}
                                        className={`p-3 sm:p-4 transition-colors ${notif.is_read ? 'bg-transparent opacity-75' : 'bg-brand-50/30 dark:bg-brand-500/5'}`}
                                    >
                                        <div className="flex gap-2 sm:gap-3">
                                            {/* Status Indicator */}
                                            <div className="mt-1 sm:mt-1.5 w-2 h-2 rounded-full shrink-0 flex items-center justify-center">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start gap-1 sm:gap-2 mb-1">
                                                    <h4 className={`text-xs sm:text-sm font-medium pr-2 sm:pr-4 ${notif.is_read ? 'text-slate-600 dark:text-slate-300' : 'text-slate-900 dark:text-white'}`}>
                                                        {notif.title}
                                                    </h4>
                                                </div>
                                                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-2">
                                                    {notif.message}
                                                </p>
                                                <div className="flex items-center justify-between gap-1 sm:gap-2">
                                                    ...
                                                    {!notif.is_read && (
                                                        <button
                                                            ...
                                                            className="text-[11px] sm:text-xs font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 flex items-center gap-1 whitespace-nowrap"
                                                        >
                                                            Mark Read
                                                        </button>
                                                    )}
                            ...
                            <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center px-4">
                                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-3">
                                    <Bell className="w-5 h-5 sm:w-6 sm:h-6 text-slate-400" />
                                </div>
                                <h4 className="font-medium text-slate-900 dark:text-white mb-1 text-sm sm:text-base">All caught up!</h4>
                    ...
                    {/* Footer */}
                    <div className="p-2 sm:p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 text-center">
